...
public interface ResultService {
    ...
    //
    Result queryByRoundId(Integer sessionId, Integer roundOrder);
    //2.2. 查询投票统计结果
    //2.2.1 该场次该轮次项目信息

    //2.3. 计算票数
    //创建一个类（。。。）(根据分组)
    List<T> countNote(Integer sessionId, Integer countId);

    //打分制:
    // 查询一个项目分数（根据sessionId，countId利用projectOrder分组）
    // 去掉最高分和最低分求出一个分数，累次求n次（项目总数）

    List<Integer> countGrade(Integer sessionId, Integer countId);
